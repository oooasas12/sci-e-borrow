...
export const DialogShowBorrowApproval = ({
  ...
  const handleGeneratePDF = async (status: string) => {
    ...
    try {
      ...
      const formData = new FormData();
      formData.append("approval_status_borrow_id", status);
      if (status === "1") {
        formData.append(
            "file",
            new File(
              [pdfBlob],
              `คำร้องขอยืมครุภัณฑ์_${data[0].borrow_list.id}.pdf`,
      ...
      await toast.promise(
        fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/borrow-list/update-status-borrow/${data[0].borrow_list.id}`,
          {
            method: "PATCH",
            body: formData,
          }
          ...
          if (!response.ok) {
            throw new Error("ไม่สามารถอัปเดตสถานะการยืมได้");
          }
          handleResponse("success");
          return response;
        }),
        {
          loading: 'กำลังอัปเดตสถานะการยืม...',
          success: 'อัปเดตสถานะการยืมสำเร็จ',
          error: 'เกิดข้อผิดพลาดในการอัปเดตสถานะการยืม'
        }
      );
              ...
              {data.length > 0 && data[0].borrow_list && (
                <div id="pdf-content" className="w-full overflow-y-auto p-12">
                  ...
                  <div className="mb-4 mt-10">
                    <p className="text-lg">
                      รายการยืมครุภัณฑ์ที่ {data[0].borrow_list.id}
                    </p>
                  </div>
