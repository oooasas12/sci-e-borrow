...
const EquipmentBrokenPage: React.FC = () => {
    ...
    const [data, setData] = useState([
        { id: 1, equipment_status_broken: 'equipment_status_broken 1' },
        { id: 2, equipment_status_broken: 'equipment_status_broken 2' },
        { id: 3, equipment_status_broken: 'equipment_status_broken 3' },
        { id: 4, equipment_status_broken: 'equipment_status_broken 4' },
        { id: 5, equipment_status_broken: 'equipment_status_broken 5' },
        { id: 6, equipment_status_broken: 'equipment_status_broken 6' },
        { id: 7, equipment_status_broken: 'equipment_status_broken 7' },
        { id: 8, equipment_status_broken: 'equipment_status_broken 8' },
    ]);
    ...
    return (
        <Layout>
            <div className='container'>
                <h1 className='title lg text-font_color'>รายการสถานะครุภัณฑ์ชำรุด</h1>
                    ...
                    <Table className='rounded-lg border'>
                        <TableHeader>
                            <TableRow className=''>
                                ...
                                <TableHead className=' whitespace-nowrap'>สถานะครุภัณฑ์ชำรุด</TableHead>
                                <TableHead className=' whitespace-nowrap'></TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredData?.slice(
                                ...
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell>{item.equipment_status_broken}</TableCell>
                                    <TableCell className='flex gap-2 justify-end' >
                                        <MdEditSquare className='text-yellow-500 cursor-pointer' onClick={() => setOpenEditData(true)} size={20} />
                                        <MdDelete className='text-red-600 cursor-pointer' onClick={() => handleDel(item.id, item.equipment_status_broken)} size={20} />
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                            ...
                            <div className='flex flex-col gap-4'>
                                <div className='flex justify-between'>
                                    <DialogTitle as="h2" className="text-base font-semibold text-gray-900">
                                        เพิ่มรายการสถานะครุภัณฑ์ชำรุด
                                    </DialogTitle>
                                    <FaXmark className=' cursor-pointer text-gray-400 hover:text-gray-600' onClick={() => setOpenInsertData(false)} />
                                </div>
                                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4'>
                                    <div className='flex flex-col gap-2'>
                                        <label htmlFor="name" className='text-sm text-font_color'>ชื่อสถานะครุภัณฑ์ชำรุด</label>
                                        <Input
                                            type="text"
                                            placeholder="ชื่อสถานะครุภัณฑ์ชำรุด"
                                        />
                                    </div>
                    ...
                    <div className="flex gap-4 min-h-full items-start justify-center p-4 sm:items-center text-center sm:p-0">
                        ...
                        >
                            <div className='flex flex-col gap-4'>
                                <div className='flex justify-between'>
                                    <DialogTitle as="h2" className="text-base font-semibold text-gray-900">
                                        ลบรายการสถานะครุภัณฑ์ชำรุด
                                    </DialogTitle>
                                    <FaXmark className=' cursor-pointer text-gray-400 hover:text-gray-600' onClick={() => setOpenDelData(false)} />
                                </div>
                                <div className='flex flex-col gap-4'>
                                    <span className='text-font_color'>คุณต้องการลบรายการสถานะครุภัณฑ์ชำรุด ชื่อ: <b>{delData.name}</b> หรือไม่</span>
