...
const StatusApprove: React.FC = () => {
    ...
    const [data, setData] = useState([
        ...
        { id: 3, status: 'status 3' },
        { id: 4, status: 'status 4' },
        { id: 5, status: 'status 5' },
        { id: 6, status: 'status 6' },
        { id: 7, status: 'status 7' },
        { id: 8, status: 'status 8' },
    ]);
    ...
    return (
        <Layout>
            <div className='container'>
                <h1 className='title lg text-font_color'>รายการสถานะการอนุมัติ</h1>
                    ...
                    <Table className='rounded-lg border'>
                        <TableHeader>
                            <TableRow className=''>
                                ...
                                <TableHead className=' whitespace-nowrap'>สถานะการอนุมัติ</TableHead>
                                <TableHead className=' whitespace-nowrap'></TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {filteredData?.slice(
                                ...
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell className='flex gap-2 justify-end' >
                                        ...
                                        <MdDelete className='text-red-600 cursor-pointer' onClick={() => handleDel(item.id, item.status)} size={20} />
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                            ...
                            <div className='flex flex-col gap-4'>
                                <div className='flex justify-between'>
                                    <DialogTitle as="h2" className="text-base font-semibold text-gray-900">
                                        เพิ่มรายการสถานะการอนุมัติ
                                    </DialogTitle>
                                    <FaXmark className=' cursor-pointer text-gray-400 hover:text-gray-600' onClick={() => setOpenInsertData(false)} />
                                </div>
                                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4'>
                                    <div className='flex flex-col gap-2'>
                                        <label htmlFor="name" className='text-sm text-font_color'>ชื่อสถานะการอนุมัติ</label>
                                        <Input
                                            type="text"
                                            placeholder="ชื่อสถานะการอนุมัติ"
                                        />
                                    </div>
                    ...
                    <div className="flex gap-4 min-h-full items-start justify-center p-4 sm:items-center text-center sm:p-0">
                        ...
                        >
                            <div className='flex flex-col gap-4'>
                                <div className='flex justify-between'>
                                    <DialogTitle as="h2" className="text-base font-semibold text-gray-900">
                                        ลบรายการสถานะการอนุมัติ
                                    </DialogTitle>
                                    <FaXmark className=' cursor-pointer text-gray-400 hover:text-gray-600' onClick={() => setOpenDelData(false)} />
                                </div>
                                <div className='flex flex-col gap-4'>
                                    <span className='text-font_color'>คุณต้องการลบรายการสถานะการอนุมัติ ชื่อ: <b>{delData.name}</b> หรือไม่</span>
