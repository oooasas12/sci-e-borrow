...
const EquipmentStatusPage: React.FC = () => {
    ...
    const [data, setData] = useState([
        { id: 1, equipment_status: 'equipment_status 1' },
        { id: 2, equipment_status: 'equipment_status 2' },
        { id: 3, equipment_status: 'equipment_status 3' },
        { id: 4, equipment_status: 'equipment_status 4' },
        { id: 5, equipment_status: 'equipment_status 5' },
        { id: 6, equipment_status: 'equipment_status 6' },
        { id: 7, equipment_status: 'equipment_status 7' },
        { id: 8, equipment_status: 'equipment_status 8' },
    ]);
                        ...
                        <TableBody>
                            {filteredData?.slice(
                                ...
                                <TableRow key={item.id}>
                                    ...
                                    <TableCell>{item.equipment_status}</TableCell>
                                    <TableCell className='flex gap-2 justify-end' >
                                        <MdEditSquare className='text-yellow-500 cursor-pointer' onClick={() => setOpenEditData(true)} size={20} />
                                        <MdDelete className='text-red-600 cursor-pointer' onClick={() => handleDel(item.id, item.equipment_status)} size={20} />
                                    </TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                            ...
                            <div className='flex flex-col gap-4'>
                                <div className='flex justify-between'>
                                    <DialogTitle as="h2" className="text-base font-semibold text-gray-900">
                                        เพิ่มรายการสถานะครุภัณฑ์
                                    </DialogTitle>
                                    <FaXmark className=' cursor-pointer text-gray-400 hover:text-gray-600' onClick={() => setOpenInsertData(false)} />
                                </div>
                                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4'>
                                    <div className='flex flex-col gap-2'>
                                        <label htmlFor="name" className='text-sm text-font_color'>ชื่อสถานะครุภัณฑ์</label>
                                        <Input
                                            type="text"
                                            placeholder="ชื่อสถานะครุภัณฑ์"
                                        />
                                    </div>
                    ...
                    <div className="flex gap-4 min-h-full items-start justify-center p-4 sm:items-center text-center sm:p-0">
                        ...
                        >
                            <div className='flex flex-col gap-4'>
                                <div className='flex justify-between'>
                                    <DialogTitle as="h2" className="text-base font-semibold text-gray-900">
                                        ลบรายการสถานะครุภัณฑ์
                                    </DialogTitle>
                                    <FaXmark className=' cursor-pointer text-gray-400 hover:text-gray-600' onClick={() => setOpenDelData(false)} />
                                </div>
                                <div className='flex flex-col gap-4'>
                                    <span className='text-font_color'>คุณต้องการลบรายการสถานะครุภัณฑ์ ชื่อ: <b>{delData.name}</b> หรือไม่</span>
