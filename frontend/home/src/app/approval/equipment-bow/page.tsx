...
const EquipmentBow: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/borrow-list/branch/${user.branch.id}`);
    ...
    return (
        <Layout>
            ...
            <DialogShowBorrowApproval title='เอกสารรายการยืม-คืน' data={selectedEquipmentBorrowEdit} onClose={closeModalDetail} open={openDetail} />
        </Layout>
    );
};
